...
export default defineConfig({
  ...
  projects: [
    { name: "setup", testMatch: /.*\.setup\.ts/, use: { ...credSafeArtifacts } },
    {
      name: "public",
      testMatch:
        /specs\/(smoke|auth|learner|life-skills-math|meaningful-(clock|money|measure|array|tenframe|fraction|order|journal|phonics|sightword|comprehension|language)|science|baseline-placement|adaptive-generation|oral-reading|typing)\.spec\.ts/,
    ...
    },
    {
      ...
      testMatch: /specs\/(parent|motivation|adaptive-generation-signed-in|oral-reading)\.spec\.ts/,
      dependencies: ["setup"],
    ...
    },
    {
      ...
      // Signs up its OWN throwaway parent account per test (see the spec), so it
      // shares no state with the seeded parent — no storageState, and it can run
      // fully in parallel again.
      dependencies: ["setup"],
      use: { ...devices["Desktop Chrome"] },
    },
    {
