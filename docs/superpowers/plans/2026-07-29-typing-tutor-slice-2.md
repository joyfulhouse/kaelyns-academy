...
- **D5 — Shared-tag lesson auto-completion inside Word Workshop is accepted**: all write lessons train `typing.words.familiar`, so one solid lesson can complete siblings. Same ruling as slice 1's Star Catch arcade (optional practice on the same skill). Do NOT invent per-lesson skills.
  - *Update (2026-07-31, PR #87):* the ruling stands — no per-lesson skills were invented — but the consequence it tolerated turned out to be worse than "siblings auto-complete". Because Word Workshop's whole skill set is a subset of Home Base's, the unit read complete and `nextBest` dropped it entirely: 0 of its 8 activities were ever offered, and Big Letters' Star Echo lesson vanished the same way. Fixed in the recommender rather than the tags — a satisfied strand still offers activities the child has never played, ranked behind anything genuinely pending.
